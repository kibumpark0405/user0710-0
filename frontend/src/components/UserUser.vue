<template>
    <v-card style="width:450px;" outlined color="surface">
        ...
        <v-card-title v-if="value._links">
            유저 # {{value._links.self.href.split("/")[value._links.self.href.split("/").length - 1]}}
        </v-card-title >
        <v-card-title v-else>
            유저
        </v-card-title >        
        ...
        <v-card-text>
            ...
            <String label="나이" v-model="value.age" :editMode="editMode" :inputUI="''"/>
            <Country offline label="국가" v-model="value.country" :editMode="editMode" @change="change"/>
        </v-card-text>
